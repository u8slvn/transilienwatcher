...
class DaemonApp(Daemon):
    def __init__(self):
        super(DaemonApp, self).__init__(pidfile=pidfile, app_name="TestApp")
...
def test_daemon_status(mocker, capsys, is_running, expected):
    mocker.patch("transilienwatcher.daemon.Daemon._is_running", return_value=is_running)
    daemon_app = DaemonApp()
